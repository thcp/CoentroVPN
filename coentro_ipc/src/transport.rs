...
impl UnixSocketListener {
    ...
    pub async fn bind_with_auth<P: AsRef<Path>>(path: P, auth_config: AuthConfig) -> IpcResult<Self> {
    ...
    pub async fn accept(&self) -> IpcResult<UnixSocketConnection> {
        ...
        let peer_cred = Self::get_peer_credentials(&stream)
            .map_err(|e| IpcError::Authentication(format!("Failed to get peer credentials: {}", e)))?;
    ...
    fn test_unix_socket_transport() {
        ...
        let server_thread = thread::spawn(move || {
            ...
            server_runtime.block_on(async {
                ...
                let listener = UnixSocketListener::bind_with_auth(&socket_path_clone, auth_config).await.unwrap();
