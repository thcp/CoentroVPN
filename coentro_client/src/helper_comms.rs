...
use coentro_ipc::transport::{UnixSocketTransport, IpcTransport, IpcResult, IpcError};
...
impl HelperClient {
    ...
    pub async fn ping(&self) -> Result<(), anyhow::Error> {
        ...
        transport.send_request(&ClientRequest::Ping).await
        ...
        match response {
            ...
            _ => {
                ...
                Err(anyhow::anyhow!("Unexpected response to ping: {:?}", response))
            }
        }
    }
    ...
    pub async fn get_status(&self) -> Result<StatusDetails, anyhow::Error> {
        ...
        transport.send_request(&ClientRequest::GetStatus).await
        ...
        match response {
            ...
            _ => {
                ...
                Err(anyhow::anyhow!("Unexpected response to status request: {:?}", response))
            }
        }
    }
