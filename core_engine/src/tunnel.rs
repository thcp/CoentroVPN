...
use shared_utils::quic::{QuicClient, QuicServer, TransportMessage, QuicTransport};
...
impl Tunnel {
    ...
    pub async fn initialize(&mut self) -> Result<(), TunnelError> {
        ...
        let connection = match &self.transport {
            TransportType::Client(client) => {
                client.connect_to_server(self.remote_addr).await
            ...
            TransportType::Server(_) => {
                ...
                return Err(TunnelError::Transport("Server tunnels don't initiate connections".to_string()));
            }
        };
    ...
    pub async fn send(&self, data: &[u8]) -> Result<(), TunnelError> {
        ...
        match &self.transport {
            TransportType::Client(client) => {
                client.send(connection.clone(), encoded).await
                    .map_err(|e| TunnelError::Transport(e.to_string()))?;
            }
            TransportType::Server(server) => {
                server.send(connection.clone(), encoded).await
                    .map_err(|e| TunnelError::Transport(e.to_string()))?;
            }
        }
    ...
    pub async fn receive(&mut self) -> Result<Vec<u8>, TunnelError> {
        ...
        let mut rx = match &self.transport {
            TransportType::Client(client) => {
                client.receive(connection.clone()).await
                    .map_err(|e| TunnelError::Transport(e.to_string()))?
            }
            TransportType::Server(server) => {
                // For server tunnels, we use the start method
                return Err(TunnelError::Transport("Server tunnels should use start() method".to_string()));
            }
        };
        ...
        match message {
            TransportMessage::Data(raw_data) => {
                ...
                let frame_count = framer.process_data(&raw_data).map_err(|e| TunnelError::Framing(e.to_string()))?;
