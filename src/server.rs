...
impl Tunnel for Server {
    async fn start(&mut self) -> Result<(), Box<dyn std::error::Error + Send + Sync>> { // mutable self
        ...
        self.socket = socket; // Assign the socket to the struct field

        let mut buf = [0u8; 1500];  // Buffer to hold incoming data
        loop {
            ...
            // Implement rate limiting: Get the rate limit from config
            ...
            // Split the packet if it exceeds the maximum size
            let mtu = self.config.mtu.unwrap_or(1500);
            let max_size = self.config.max_packet_size.unwrap_or_else(|| calculate_max_payload_size(mtu.into()));
            let chunks = self.split_packet(&buf[..len], max_size); // Now using the split_packet method in Server

            // Spawn a new task for handling each packet chunk concurrently
            ...
            for chunk in chunks {
                ...
                tokio::spawn(async move {
                    // Lock the socket to send data back to the peer
    ...
    async fn send_data(&self, data: &[u8], addr: SocketAddr) -> Result<(), Box<dyn std::error::Error>> {
        ...
        // Split the data if it exceeds the maximum size
        let mtu = self.config.mtu.unwrap_or(1500);
        let max_size = self.config.max_packet_size.unwrap_or_else(|| calculate_max_payload_size(mtu.into()));
        let chunks = self.split_packet(data, max_size);
