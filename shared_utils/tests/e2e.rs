...
async fn test_config_to_tunnel_e2e() {
    ...
    let server_tunnel_id = tunnel_manager.create_tunnel_from_config(config).await.unwrap();
    ...
    // Clean up
    tunnel_manager.close_tunnel(&client_tunnel_id).await.unwrap();
    tunnel_manager.close_tunnel(&server_tunnel_id).await.unwrap();
}
...
async fn test_large_message_fragmentation_and_reassembly() {
    ...
    let actual_server_addr = timeout(Duration::from_secs(5), actual_addr_rx).await
...
async fn test_tampered_payload_decryption_failure() {
    ...
    assert!(decrypt_result.is_err(), "Decryption should fail for tampered data");
    
    println!("Tampered payload correctly rejected: {}", decrypt_result.unwrap_err());
...
async fn test_multiple_concurrent_client_sessions() {
    ...
    println!("Successfully connected {} out of {} clients", successful_connections, MAX_CONCURRENT_CLIENTS);
...
async fn run_large_message_client(server_addr: SocketAddr, key: &[u8], message: Vec<u8>) -> anyhow::Result<()> {
...
async fn run_encrypted_quic_client(server_addr: SocketAddr, key: &[u8], messages: Vec<&str>) -> anyhow::Result<()> {
