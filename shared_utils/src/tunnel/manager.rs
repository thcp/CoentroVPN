...
impl TunnelManager {
    ...
    fn add_tunnel(&self, handle: TunnelHandle) -> TunnelResult<()> {
        ...
        self.tunnels.lock().unwrap().insert(id.clone(), handle_arc.clone());
    ...
    fn start_tunnel_processing(
        ...
        tokio::spawn(async move {
            ...
            let connection_opt = { // Removed mut
            ...
            if let Some(mut conn) = connection_opt {
                loop {
                    ...
                    if matches!(current_state, TunnelState::Disconnecting | TunnelState::Disconnected | TunnelState::Failed) {
                    ...
                    match conn.recv_data().await {
                        ...
                        Ok(None) => {
                            ...
                            handle_arc.lock().unwrap().set_state(TunnelState::Disconnected);
                            break;
                        }
                        Err(e) => {
    ...
    pub async fn send_data(&self, id: &TunnelId, data: Vec<u8>) -> TunnelResult<()> {
        let tunnel_handle_arc = self.get_tunnel(id).ok_or_else(|| {
        ...
        if let Some(conn) = handle_guard.connection.as_mut() {
            conn.send_data(&data).await.map_err(TunnelError::Transport)?;
    ...
    pub async fn close_tunnel(&self, id: &TunnelId) -> TunnelResult<()> {
        ...
        { // Scope for the lock
            ...
            if handle.state != TunnelState::Disconnecting && handle.state != TunnelState::Disconnected {
