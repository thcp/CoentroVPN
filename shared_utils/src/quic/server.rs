...
impl QuicServer {
    ...
    pub fn new(bind_addr: SocketAddr, key: &[u8]) -> TransportResult<Self> {
        ...
        // Initialize cipher
        let cipher = Arc::new(AesGcmCipher::new(key).map_err(|e| TransportError::Other(format!("Failed to initialize server cipher: {}", e)))?);
                        ...
                        tokio::spawn(async move {
                            match conn.await {
                                Ok(connection) => {
                                    ...
                                    if let Err(e) = Self::handle_connection(connection, tx_clone, cipher_clone).await {
        ...
        loop {
            ...
            match connection.accept_bi().await {
                Ok((send, recv)) => {
                    ...
                    tokio::spawn(async move {
                        if let Err(e) = Self::handle_bidirectional_stream(send, recv, tx_clone, cipher_clone).await {
                ...
                Ok(Some(chunk)) => {
                    ...
                    match cipher.decrypt(&chunk.bytes) {
                        Ok(decrypted_data) => {
                            ...
                            if let Err(e) = tx.send(TransportMessage::Data(decrypted_data.clone())).await { // Clone data if used again
                            ...
                            if false { // Disabled echo for now to avoid errors when client closes stream
                                debug!("Encrypting {} bytes before echoing to client", decrypted_data.len());
                                match cipher.encrypt(&decrypted_data) {
                                    Ok(encrypted_response) => {
                                        debug!("Encrypted to {} bytes for echoing", encrypted_response.len());
                                        // Try to write, but don't fail the whole stream if it doesn't work
                                        if let Err(e) = send.write_all(&encrypted_response).await {
                                            debug!("Could not echo back to client (likely stream closed): {}", e);
                        ...
                        Err(e) => {
                            ...
                            let _ = tx.send(TransportMessage::Error(TransportError::Other(format!("Server decryption failed: {}", e)))).await;
...
impl QuicTransport for QuicServer {
    ...
    async fn send(&self, connection: Connection, data: Vec<u8>) -> TransportResult<()> {
        ...
        debug!("Encrypted to {} bytes for sending from server", encrypted_data.len());
