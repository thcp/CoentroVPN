...
fn print_config(config: &Config) {
    ...
    println!("Security:");
    println!("  PSK: {}", config.security.psk.as_deref().unwrap_or("<not set>"));
    println!("  Cert Path: {}", config.security.cert_path.as_deref().unwrap_or("<not set>"));
    println!("  Key Path: {}", config.security.key_path.as_deref().unwrap_or("<not set>"));
    println!("  Verify TLS: {}", config.security.verify_tls);
    
    match config.role {
        Role::Client => {
            println!("Client Configuration:");
            println!("  Server Address: {}", config.client.server_address.as_deref().unwrap_or("<not set>"));
            println!("  Auto Reconnect: {}", config.client.auto_reconnect);
            println!("  Reconnect Interval: {} seconds", config.client.reconnect_interval);
        }
        Role::Server => {
            println!("Server Configuration:");
            println!("  Virtual IP Range: {}", config.server.virtual_ip_range.as_deref().unwrap_or("<not set>"));
